...
public class BtHelper {
    ...
    public ReadRunnable readRunnable;

    //设置是否记录信息的信号
    public void setRecord(boolean flag) {
        if(null != readRunnable) {
            readRunnable.setRecord(flag);
        }
    }
    ...
    public int getSaveState() {
        if(null != readRunnable) {
            return readRunnable.getSaveState();     //1就是正在记录中，0就是处于停止中
        }else {
            return -1;      //获取状态失败
        }
    }
    private void receiveMessage() {
        ...
        OnReceiveListener onReceiveListener = new OnReceiveListener() {
            @Override
            public void onNewLine(Bundle dataBundle) {
                Intent intent = new Intent(BroadcastType.BROADCAST_TYPE_RECEIVED_MESSAGE);
                intent.putExtra("message", dataBundle);
                mContext.sendBroadcast(intent);
            }
        ...
        //ReadRunnable readRunnable;        //暂时暴露接口
        if( type == com.houxya.bthelper.Constants.CONNECT_TYPE_CLIENT){
            readRunnable = new ReadRunnable(onReceiveListener, mConnectDeviceRunnable.getInputStream());
            mExecutorService.submit(readRunnable);
        }else if( type == com.houxya.bthelper.Constants.CONNECT_TYPE_SERVER){
            readRunnable = new ReadRunnable(onReceiveListener, mAcceptRunnable.getInputStream());
            mExecutorService.submit(readRunnable);
        }
